...
signal monster_detected
signal monster_lost

func _ready() -> void:
	last_footprint_pos = global_position
	print("Chase node:", chase_music)
	for item in get_tree().get_nodes_in_group("Collectibles"):
		item.collected.connect(self._on_collected_item)

func _on_collected_item(item):
